...
const HomeScreen = props => {
  ...
  const {
    ...
    data: screenData,
    error: screenError,
  } = DraftbitExampleDataApi.useGET10MostRecentArticles17421GET();
  ...
  return (
    ...
    >
      <>
        {!bannerTopIsActive(Constants['configFileJson']) ? null : (
          <Touchable
            onPress={() => {
              try {
                navigation.navigate('CampaingNavigator', {
                  screen: 'CampaingScreen',
                  params: { idCampaing: tabView1, title: 'example title' },
                });
              } catch (err) {
                console.error(err);
              }
      ...
      <Touchable
        onPress={() => {
          try {
            navigation.navigate('OtherNavigator', { screen: 'SearchScreen' });
          } catch (err) {
            console.error(err);
          }
        }}
            ...
            return (
              <Touchable>
                <View style={styles.View_5e} pointerEvents={'auto'}>
                  ...
                  />
                  <Text
                    style={[styles.TextXq, { color: theme.colors.strong }]}
                    ellipsizeMode={'clip'}
                    numberOfLines={1}
                  >
